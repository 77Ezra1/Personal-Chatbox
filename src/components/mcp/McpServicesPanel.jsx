...
export default function McpServicesPanel() {
  const { services, loading, error, toggleService, refresh } = useMcpManager()
  ...
  }, [refresh, reloadUserConfigs])

  const handleToggleServer = async (service) => {
    try {
  ...
  const handleAddService = async (templateId, envVars) => {
    try {
      ...
      // 提示成功
      alert(translate('mcp.common.addSuccess', 'MCP 服务添加成功！'))
    } catch (err) {
      ...
      <div className="flex items-center justify-center p-12">
        <div className="text-center">
          ...
          <p className="text-sm text-muted-foreground">
            {translate('mcp.common.loading', '加载服务列表中...')}
          </p>
        </div>
      </div>
    )
  ...
  if (error) {
    return (
      <div className="p-4 flex items-center gap-2 text-destructive bg-destructive/10 rounded-lg">
        ...
        <div>
          <p className="font-medium">{translate('mcp.common.loadFailedTitle', '加载失败')}</p>
    ...
    <div className="space-y-6">
      ...
      <div className="text-sm text-muted-foreground space-y-1">
        ...
        <p className="flex items-center gap-2">
          <span className="inline-flex items-center gap-1 text-xs bg-primary/10 text-primary px-2 py-0.5 rounded">
            ...
            {translate('mcp.servicesPanel.stats', {
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        ...
        <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto flex-1">
          ...
          >
            ...
            {translate('mcp.servicesPanel.addButton', '添加服务')}
          </button>
          {/* 筛选按钮组 */}
          <div className="inline-flex rounded-lg border p-1 bg-muted/30">
            ...
            >
              {translate('mcp.servicesPanel.filterAll', { count: stats.total })}
            </button>
            <button
            ...
            >
              {translate('mcp.servicesPanel.filterSystem', { count: stats.systemCount })}
            </button>
            <button
            ...
            >
              {translate('mcp.servicesPanel.filterCustom', { count: stats.customCount })}
            </button>
          </div>
          ...
          <div className="relative flex-1 sm:max-w-xs">
            ...
            <Input
              placeholder={translate('mcp.servicesPanel.searchPlaceholder', '搜索服务...')}
              value={searchQuery}
        ...
        <div className="inline-flex rounded-lg border p-1 bg-muted/30">
          <button
            ...
            title={translate('mcp.servicesPanel.gridView', '卡片视图')}
          >
          ...
          <button
            ...
            title={translate('mcp.servicesPanel.listView', '列表视图')}
          >
      ...
      {filteredServices.length === 0 ? (
        <div className="text-center py-12 text-muted-foreground">
          ...
          <p>{translate('mcp.servicesPanel.emptyTitle', '没有找到匹配的服务')}</p>
          <p className="text-sm mt-1">{translate('mcp.servicesPanel.emptyHint', '尝试调整筛选条件或搜索关键词')}</p>
        </div>
      ) : (
        <div className={
          viewMode === 'grid'
          ...
          <div className="flex items-center gap-2">
            ...
            {!isSystemService && (
              <Badge variant="outline" className="text-xs">
                {translate('mcp.common.badgeCustom', '自定义')}
              </Badge>
            )}
          ...
          {service.tools && service.tools.length > 0 && (
            <Badge variant="outline" className="text-xs">
              {translate('mcp.common.badgeTools', '{count} 个工具')
                .replace('{count}', service.tools.length.toString())}
            </Badge>
          )}
        </div>
